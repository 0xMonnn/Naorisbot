...
class NaorisProtocol:
    ...
    def log(self, message):
        print(
            f"{Fore.BLUE}{datetime.now().astimezone(wib).strftime('%x %X %Z')}{Style.RESET_ALL}"
...
"""
    ...
    async def load_proxies(self):
        ...
        try:
            if not os.path.exists(filename):
                self.log(
                    f"⚠️  File {filename} not found.{Fore.MAGENTA} Running without proxies{Style.RESET_ALL}"
                )
                return False
            ...
            if not self.proxies:
                self.log(
                    f"⚠️  No proxies found in file.{Fore.MAGENTA} Running without proxies{Style.RESET_ALL}"
                )
                return False

            self.log(
                f"🔄  Proxies loaded {Fore.MAGENTA}Successfully: {Style.RESET_ALL}"
            ...
            self.log(
                f"⚠️  Failed to load proxies: {e}. {Fore.MAGENTA} Running without proxies{Style.RESET_ALL}"
            )
            self.proxies = []
            return False
        ...
        try:
            with open("accounts.json", "r") as file:
                ...
                if accounts:
                    ...
                    self.log(
                        f"✅ Successfully loaded {Fore.YELLOW}#{len(accounts)}{Fore.RESET} accounts from {Fore.GREEN}accounts.json{Style.RESET_ALL}"
                    )
                    return accounts
                else:
                    self.log(
                        f"❌ No accounts found in {Fore.RED}accounts.json{Style.RESET_ALL}"
                    )
                    return []
            ...
            self.log(
                f"{Fore.RED }❌ Invalid JSON format in accounts.json{Style.RESET_ALL}"
            )
            return []
            ...
            try:
                ...
                if response.status_code == 404:
                    return self.print_message(
                        ...
                        proxy is not None,
                        Fore.RED,
                        f"❌ Authentication Failed: Join Testnet & Complete Required Tasks First",
                    )
            ...
            try:
                ...
                if response.status_code == 401:
                    ...
                    return self.print_message(
                        ...
                        proxy is not None,
                        Fore.YELLOW,
                        f"⚠️  Whitelist Status: Already {Fore.MAGENTA}In Whitelist",
                    )
        ...
        for attempt in range(retries):
            try:
                ...
                return self.print_message(
                    ...
                    proxy is not None,
                    Fore.RED,
                    f"❌ Protection Toggle Failed: {str(e)}",
                )
            ...
            try:
                ...
                if "502" in str(e):
                    return self.print_message(
                        ...
                        proxy is not None,
                        Fore.RED,
                        f"❌ PING Failed: {Fore.RED}Server Down",
                    )
    ...
    async def process_get_access_token(
        ...
        while token is None:
            ...
            self.print_message(
                ...
                proxy is not None,
                Fore.GREEN,
                f"✅ Authentication {Fore.GREEN}Successful",
            )
            return token
            ...
            if time_diff >= self.earnings_report_interval:
                ...
                if wallet:
                    ...
                    self.print_message(
                        ...
                        proxy is not None,
                        Fore.BLUE,
                        f"🤑 Earnings-Today: {Fore.GREEN}{today_earning} Points | "
    ...
    async def process_activate_toggle(
        ...
        if whitelist and whitelist.get("message") == "url saved successfully":
            self.print_message(
                ...
                proxy is not None,
                Fore.GREEN,
                f"✅ Added to Whitelist {Fore.GREEN}Successfully",
            )
            ...
            if deactivate and deactivate.strip() == "No action needed":
                ...
                if activate and activate.strip() == "Session started":
                    self.print_message(
                        ...
                        proxy is not None,
                        Fore.GREEN,
                        f"✅ Protection Activated {Fore.GREEN}Successfully",
                    )
                    return True
                else:
                    continue
                ...
                for i in range(num_pings):
                    ...
                    if heartbeat:
                        self.print_message(
                            ...
                            proxy is not None,
                            Fore.GREEN,
                            f"✅ PING!!! #{ping_counter} {Fore.GREEN}Successful",
                        )
    ...
    async def main(self):
        try:
            ...
            if not accounts:
                self.log(
                    f"❌ No accounts loaded. Please check your {Fore.RED}accounts.json file.{Style.RESET_ALL}"
                )
                return
            ...
            self.log(
                f"🚀 Starting Naoris Protocol Testnet with {Fore.YELLOW}#{len(accounts)}{Style.RESET_ALL} accounts"
            )
            self.log(
                f"{Fore.MAGENTA}ℹ️  Using {'proxies' if use_proxy else 'local connection'}{Style.RESET_ALL}"
            )
            self.log(
                f"🕐 PING interval in {Fore.YELLOW}{self.ping_interval} seconds{Style.RESET_ALL} with random {Fore.GREEN}{self.ping_frequency[0]}-{self.ping_frequency[1]} pings{Style.RESET_ALL} per interval"
            )
            self.log(
                f"📊 Earnings point report interval in {Fore.YELLOW}{self.earnings_report_interval} seconds (one hour){Style.RESET_ALL}"
            )
...
if __name__ == "__main__":
    try:
        ...
        print(
            ...
            f"{Fore.YELLOW}👋 Shutting down Naoris Protocol Testnet{Style.RESET_ALL}"
        )
