...
"""
        ...
        try:
            if use_proxy_choice == 1:
                response = await asyncio.to_thread(
                    requests.get, "https://raw.githubusercontent.com/monosans/proxy-list/main/proxies/all.txt"
                )
    ...
    async def user_login(self, address: str, proxy=None, retries=5):
        ...
        for attempt in range(retries):
            try:
                response = await asyncio.to_thread(
                    requests.post, url=url, headers=headers, data=data, proxy=proxy, timeout=60, impersonate="safari15_5"
                )
                if response.status_code == 404:
                    return self.print_message(
                        self.mask_account(address), proxy, Fore.RED,
                        f"GET Access Token Failed: {Fore.YELLOW + Style.BRIGHT}Join Testnet & Complete Required Tasks First"
                    )
                ...
                return self.print_message(
                    self.mask_account(address), proxy, Fore.RED,
                    f"GET Access Token Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}"
                )
    ...
    async def wallet_details(self, address: str, token: str, use_proxy: bool, proxy=None, retries=5):
        ...
        for attempt in range(retries):
            try:
                ...
                return self.print_message(
                    self.mask_account(address), proxy, Fore.RED,
                    f"GET Wallet Details Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}"
                )
        ...
        for attempt in range(retries):
            try:
                ...
                if response.status_code == 401:
                    ...
                    return self.print_message(
                        self.mask_account(address), proxy, Fore.RED,
                        f"Add to Whitelist Failed: {Fore.YELLOW + Style.BRIGHT}Already Exists In Whitelist"
                    )
                ...
                return self.print_message(
                    self.mask_account(address), proxy, Fore.RED,
                    f"Add to Whitelist Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}"
                )
    ...
    async def toggle_activated(self, address: str, token: str, state: str, device_hash: int, proxy=None, retries=5):
        ...
        for attempt in range(retries):
            try:
                ...
                return self.print_message(
                    self.mask_account(address), proxy, Fore.RED,
                    f"Turn On Protection Failed: {Fore.YELLOW + Style.BRIGHT}{str(e)}"
                )
    ...
    async def send_heartbeats(self, address: str, token: str, use_proxy: bool, proxy=None, retries=5):
        ...
        for attempt in range(retries):
            try:
                response = await asyncio.to_thread(
                    requests.post, url=url, headers=headers, json={}, proxy=proxy, timeout=60, impersonate="safari15_5"
                )
                if response.status_code == 401:
                    token = await self.process_get_access_token(address, use_proxy)
                    headers["Authorization"] = f"Bearer {token}"
                ...
                if "502" in str(e):
                    return self.print_message(
                        self.mask_account(address), proxy, Fore.RED,
                        f"PING Failed: {Fore.YELLOW + Style.BRIGHT}Server Down"
                    )
